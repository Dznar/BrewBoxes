...
export default async function handler(req: any, res: any) {
  ...
  try {
    const deleteCommand = `${detectedEngine} rm -f ${containerId}`;
    await execCommandWithOutput(deleteCommand);

    res.status(200).json({ success: true, message: 'Container deleted successfully' });
  } catch (error) {
    res.status(500).json({ success: false, message: error.message || 'Failed to delete container' });
  }
}
